...
namespace SaltContract\Controllers;
 ...
 *  )
 */
class NestedFilesController extends Controller
{
    protected $modelNamespace = 'SaltContract';
    ...
    public function index(Request $request, $parentId = null) {
        ...
        try {
            ...
            if($this->is_nested === true) {
                $count = $this->model->where('foreign_table', 'contracts')->where($this->parent_field, $parentId)->count();
                $model = $this->model->where('foreign_table', 'contracts')->where($this->parent_field, $parentId)->filter();
            }
